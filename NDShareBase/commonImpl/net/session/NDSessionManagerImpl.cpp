...
NDBool NDSessionManagerImpl::sendData( NDProtocol& rProtocol, NDUint32 nSessionID, NDUint8 nProDataHeadBitWise, NDBool bClient )
{
	...
	{
		NDByteBufferPool::getInstance()->releaseByteBuffer( pMsgBuf );
		return NDFalse;
	}
...
void NDSessionManagerImpl::releaseServerSession( NDUint32 nSessionID )
{
	...
	if ( iterFind != m_serverSessionMap.end() )
	{
		do 
		{
			...
			m_releaseServerSessionMap.insert( std::make_pair( iterFind->second, ( NDShareBaseGlobal::nd_getCurSecondTimeOfUTC() + SOCKET_GC_TIMEOUT)) );
		} while (0);
...
void NDSessionManagerImpl::updateReleaseSessionMap()
{
	...
	NDTime tCurrentTime = NDShareBaseGlobal::nd_getCurSecondTimeOfUTC();
	NDSession* pSession	= NULL;
...
void NDSessionManagerImpl::updateServerSessionMap()
{
	...
	NDTime nCurrentANSISecondTime = NDShareBaseGlobal::nd_getCurSecondTimeOfUTC();
	...
	for ( ;	iterBegin != iterEnd; ++iterBegin )
	{
		...
		if ( NULL != pSession )
		{
			if ( SOCKET_PING_TIMEOUT <= ( NDShareBaseGlobal::nd_getDiffMilliSecondTime( nCurrentANSISecondTime, pSession->getLastPingSecondTime() ) ) )
			{
