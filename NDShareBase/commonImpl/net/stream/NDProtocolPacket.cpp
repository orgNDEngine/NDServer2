...
NDBool NDProtocolPacket::composePacket( NDByteBuffer& outMsgBuf, NDProtocol& protocol, NDUint8 nProDataHeadBitWise )
{
	...
	if ( NDTrue == bCompression )
	{
		...
		{
			NDByteBufferPool::getInstance()->releaseByteBuffer( pProtocolBuf );
			return NDFalse;
		}
		...
		NDBool bRet = NDShareBaseGlobal::nd_compress( (char*)(outMsgBuf.ReadBuffer() + nDataHeadSize + nExtraHeadSize), &nCompression, (const char*)pProtocolBuf->ReadBuffer(), nOriginalSize );
		if ( NDFalse == bRet )
		{
			NDByteBufferPool::getInstance()->releaseByteBuffer( pProtocolBuf );
			return NDFalse;
	...
	if ( NDTrue == bCrc )
	{
		...
		nCRC32 = NDShareBaseGlobal::nd_crc32( nCRC32, (const char*)(outMsgBuf.ReadBuffer() + nDataHeadSize + nExtraHeadSize), dataHeader.m_nBodySize - nExtraHeadSize );
	}
...
NDBool NDProtocolPacket::parsePacket( NDByteBuffer& decryptBuf, NDByteBuffer& encryptBuf, const NDProDataHead& dataHeader )
{
	...
	{
		NDUint32 nCRC = 0;
		nCRC = NDShareBaseGlobal::nd_crc32( nCRC, (const char*)(pEncryptBuf + nSessionIDSize) , dataHeader.m_nBodySize - nExtraHeadSize );
		if ( nCRC != nRecvCRC )
		{
			return NDFalse;
		}
	...
	if ( bCompression )
	{
		...
		NDBool bRet = NDShareBaseGlobal::nd_uncompress((char*)(decryptBuf.ReadBuffer()+nSessionIDSize), &nUnCompression, (const char*)(pEncryptBuf + nSessionIDSize), dataHeader.m_nBodySize - nExtraHeadSize );
		if ( NDTrue == bRet )
		{
