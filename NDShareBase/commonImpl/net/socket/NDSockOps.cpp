...
NDBool NDSockOps::setRecvBufferSize( SOCKET* pFd , UINT size)
{
	if ( (NULL == pFd) || (INVALID_SOCKET == *pFd) ) return NDFalse;
...
NDBool NDSockOps::setSendBufferSize( SOCKET* pFd , UINT size)
{
	if ( (NULL == pFd) || (INVALID_SOCKET == *pFd) ) return NDFalse;
...
#else
	return ( 0 == ::getsockopt( s, level, optname, (void*)optval, optlen ) );
#endif	
}
