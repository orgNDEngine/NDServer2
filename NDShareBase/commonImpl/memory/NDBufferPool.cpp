...
void NDBufferPool::free( void* ptr )
{
	...
	assert(NDShareBaseGlobal::nd_isValidPointer(ptr));
...
{
	char szBuf[128] = {0};
	_snprintf( szBuf, 127, "preallocated shared buffers: %5d/%d x %db", (maxTinyBuffers - tinyPoolSize), maxTinyBuffers, tinyBufferSize );
	return string(szBuf);
}
