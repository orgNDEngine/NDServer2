...
NDBool NDXmlConfigServerCommon::readSerChildInfoXmlCfgCommonContent( const TiXmlElement *pParentElement )
{
	...
	if ( NULL != pContent )
	{
		m_pLocalServerInfo->setDumpPath( pContent );
	}
	else
	{
		string strDumpPath( NDShareBaseGlobal::nd_getMoudlePath() );
		strDumpPath += "/Dump/";

		m_pLocalServerInfo->setDumpPath( strDumpPath.c_str() );
	}

	pContent = getElement2( pParentElement, SERINFO_LOGPATH );
	if ( NULL != pContent )
	{
		m_pLocalServerInfo->setLogPath( pContent );
	}
	else
	{
		string strLogPath( NDShareBaseGlobal::nd_getMoudlePath() );
		strLogPath += "/log/";

		m_pLocalServerInfo->setLogPath( strLogPath.c_str() );
	}
